<script lang="ts">
	import type { Snippet } from 'svelte';

	type Variant = 'default' | 'secondary' | 'destructive' | 'outline';
	type Size = 'default' | 'sm' | 'lg';
	type Faction = 'none' | 'alliance' | 'horde';
	type Shape = 'default' | 'shield' | 'banner';

	interface BadgeProps {
		variant?: Variant;
		size?: Size;
		faction?: Faction;
		shape?: Shape;
		class?: string;
		children?: Snippet;
		[key: string]: unknown;
	}

	const variantClasses: Record<Variant, string> = {
		default:
			'text-amber-100 [text-shadow:0_0_8px_rgba(251,191,36,0.4)] wc-btn-border-frame [border-image-slice:16_fill]',
		secondary: 'text-slate-200 wc-btn-border [border-image-slice:16_fill]',
		destructive:
			'text-red-200 [text-shadow:0_0_8px_rgba(239,68,68,0.4)] wc-btn-border-frame brightness-75 hue-rotate-[320deg] [border-image-slice:16_fill]',
		outline: 'border-amber-900/50 bg-black/40 text-amber-200/80 hover:bg-black/60'
	};

	const sizeClasses: Record<Size, string> = {
		default: 'px-3 py-1 text-xs border-[3px]',
		sm: 'px-2 py-0.5 text-[10px] border-[2px]',
		lg: 'px-4 py-1.5 text-sm border-[4px]'
	};

	const factionClasses: Record<Faction, string> = {
		none: '',
		alliance:
			'text-blue-100 [text-shadow:0_0_10px_rgba(59,130,246,0.5)] border-blue-900/50 bg-blue-950/40',
		horde: 'text-red-100 [text-shadow:0_0_10px_rgba(239,68,68,0.5)] border-red-900/50 bg-red-950/40'
	};

	const shapeClasses: Record<Shape, string> = {
		default: 'rounded-md',
		shield: 'rounded-t-sm rounded-b-xl border-b-[5px]',
		banner: 'rounded-none clip-path-banner'
	};

	let {
		variant = 'default',
		size = 'default',
		faction = 'none',
		shape = 'default',
		class: className,
		children,
		...rest
	}: BadgeProps = $props();
</script>

<div
	class={[
		'fantasy inline-flex items-center gap-1.5 border-solid transition-all duration-100 [border-image-repeat:stretch] focus:ring-2 focus:ring-amber-500/50 focus:outline-none',
		variantClasses[variant],
		sizeClasses[size],
		factionClasses[faction],
		shapeClasses[shape],
		className
	]}
	{...rest}
>
	{#if children}
		{@render children()}
	{/if}
</div>
